...
export const seed = async ({
  ...
  payload.logger.info(`— Seeding blog categories...`)

  const [guidesCategory, newsCategory, storiesCategory] = await Promise.all([
    payload.create({
      collection: 'post-categories',
      data: {
        title: 'Vodiči',
        slug: 'vodici',
        description: 'Praktični članci za planiranje, strukturu i vođenje ecommerce sadržaja.',
      },
    }),
    payload.create({
      collection: 'post-categories',
      data: {
        title: 'Novosti',
        slug: 'novosti',
        description: 'Novosti, razmišljanja i kratka zapažanja iz ecommerce i content sveta.',
      },
    }),
    payload.create({
      collection: 'post-categories',
      data: {
        title: 'Priče iz prakse',
        slug: 'price-iz-prakse',
        description: 'Konkretni primeri, iskustva i male studije slučaja iz rada na starterima.',
      },
    }),
  ])
  ...
  await Promise.all([
    payload.create({
      ...
      depth: 0,
      data: homePageData({
        featuredProducts: [productHat, productTshirt],
        heroImage: imageHero,
        metaImage: imageHat,
      }),
    }),
  ...
  for (const postData of createBlogPostsSeed({
    editorialImage: imageHat,
    heroImage: imageHero,
    productImage: imageTshirtBlack,
    categories: {
  ...
  await Promise.all([
    payload.updateGlobal({
      ...
      data: {
        ...
        promoBanner: {
          enabled: true,
          text: 'Nova kolekcija i svež seed sadržaj su spremni za uređivanje.',
          link: {
            type: 'custom',
            label: 'Pogledaj shop',
            url: '/shop',
        ...
        sections: [
          {
            ...
            tagline: 'Payload Ecommerce Starter',
            description:
              'Seedovan footer sa realnim sekcijama, linkovima i newsletter blokom koji možeš odmah da prilagodiš klijentu.',
          },
          {
            blockType: 'footerColumn',
            title: 'Navigacija',
            links: [
              { link: { type: 'custom', label: 'Početna', url: '/' } },
          ...
          },
          {
            ...
            heading: 'Prijavi se na novosti',
            description:
              'Iskoristi ovu seed sekciju za newsletter, akcije ili korisne objave iz bloga.',
            buttonLabel: 'Prijavi se',
            privacyNote: 'Slanjem forme prihvataš osnovna pravila privatnosti.',
          },
        ],
        bottomBar: {
          copyright: '© 2026 Payload Ecommerce Starter. Sva prava zadržana.',
          legalLinks: [
            { link: { type: 'custom', label: 'Politika privatnosti', url: '/contact' } },
            { link: { type: 'custom', label: 'Uslovi koriscenja', url: '/contact' } },
          ],
        },
      },
    }),
