...
export function CountryGate() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const mList = fetchedMarkets.status === 'fulfilled' ? fetchedMarkets.value : []
      const countryCode = geoData.status === 'fulfilled' ? geoData.value?.country_code : null
  ...
  return (
    <div className="bs-overlay-in fixed inset-0 z-9999 flex items-center justify-center bg-black/95">
      ...
      <div className="bs-card-in relative z-10 flex w-full max-w-lg flex-col items-center px-6 py-14">
        <div className="bs-logo-wrap relative mb-8 w-full overflow-hidden">
          <Image
            src="/logo.jpeg"
        ...
        <h1 className="bs-rise-in mb-2 text-center text-2xl font-bold tracking-tight text-white" style={{ animationDelay: '0.34s' }}>
          Select your market
        </h1>
        <p className="bs-rise-in mb-10 text-center text-sm text-zinc-500" style={{ animationDelay: '0.42s' }}>
        ...
        {loading ? (
          ...
          <>
            {suggested && (
              <>
                ...
                >
                  ...
                  <span className="text-xs font-normal text-zinc-400">Shop →</span>
                </button>
                {others.length > 0 && (
                  <p className="mb-2 text-[10px] font-medium tracking-[0.3em] text-zinc-600 uppercase">
                    Other regions
            ...
            {!markets.length && (
              <p className="text-sm text-zinc-500">No markets configured yet.</p>
            )}
          </>
        )}
